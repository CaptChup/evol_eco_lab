...
#outputs a signal of desired frequency (with 1st and 2nd harmonic of desired amplitude (expressed as a fraction of signal amplitude)), syllable duration, inter-syllable period and inter-chirp period using the given audio envelopes
...
timeres = 1/sr
amplitude_ratios = float(input("Enter the fraction of amplitude you want the first harmonic to have\n")), float(input("Enter the fraction of amplitude you want the second harmonic to have\n"))

timeline = np.linspace(0, 1, num = int(sr*1), endpoint=False)
sine_wave_signal= np.sin(2*np.pi*frequency*timeline)
sine_wave_harmonic_1 = np.sin(2*np.pi*2*frequency*timeline)
sine_wave_harmonic_2 = np.sin(2*np.pi*3*frequency*timeline)
...
for i in range(0, len(envelopes)):
    syllables_list.append(np.array(envelopes[i])*sine_wave_signal[:len(envelopes[i])] + amplitude_ratios[0]*np.array(envelopes[i])*sine_wave_harmonic_1[:len(envelopes[i])] + amplitude_ratios[1]*np.array(envelopes[i])*sine_wave_harmonic_2[:len(envelopes[i])])
...
write(r"D:\Chup\academic\niser\eco-evo_lab\codes\gryllodes_signal_generator\syn_signals\merged_192k_with_python.wav", sr, syn_signal_audio)
